export function createInvitationHandler(env, fetcher = fetch, localSend) {
  ...
  return async (req) => {
    ...
    if (localSend && ["http://localhost:4321", "http://127.0.0.1:4321", "http://127.0.0.1:5192"].includes(origin)) {
    ...
    const reply = (status, message) =>
      new Response(JSON.stringify({ message }), { status, headers: requestHeaders });
    ...
    try {
      ...
      const prepared = await fetcher(
        `${base}/rest/v1/rpc/prepare_boat_invitation_email`,
        {
          method: "POST",
          headers: {
            apikey: key,
      ...
      const marked = await fetcher(
        `${base}/rest/v1/rpc/mark_boat_invitation_sent`,
        {
          method: "POST",
          headers: {
            apikey: key,
