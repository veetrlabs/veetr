...
alter table public.race_results drop constraint race_results_status_check;
alter table public.race_results add constraint race_results_status_check check(status in ('FINISHED','DNS','DNF','DSQ','RET','OCS','SCORED'));
do $$ begin
 execute replace(pg_get_functiondef('public.save_series_snapshot(jsonb,integer,uuid)'::regprocedure), '(item->>''date'')::date', 'nullif(item->>''date'','''')::date');
end $$;
