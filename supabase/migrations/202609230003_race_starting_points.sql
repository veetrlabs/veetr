...
-- The race winner's series score is an integer, including negative scores.
create or replace function public.validate_race_starting_points() returns trigger
language plpgsql set search_path = '' as $$
declare e jsonb;
begin
  for e in select value from jsonb_array_elements(new.document->'events') loop
    if e ? 'startingPoints' then
      if jsonb_typeof(e->'startingPoints') <> 'number' then
        raise exception 'Invalid race starting points';
      end if;
      if (e->>'startingPoints')::numeric <> trunc((e->>'startingPoints')::numeric)
         or abs((e->>'startingPoints')::numeric) > 100 then
        raise exception 'Invalid race starting points';
      end if;
    end if;
  end loop;
...
$$;
create trigger validate_race_starting_points before insert or update on public.series
for each row execute function public.validate_race_starting_points();
