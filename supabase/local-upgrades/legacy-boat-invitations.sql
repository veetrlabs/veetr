-- Local-only upgrade from the older creation_access migration history.
-- Keeps the original history intact; records the actual compatibility work below.
-- Run with psql -v ON_ERROR_STOP=1. Refuses other starting schemas.
begin;
do $$ begin
 if not exists(select 1 from supabase_migrations.schema_migrations where version='202609130001' and name='creation_access')
 or to_regclass('public.tracking_sessions') is not null
 or to_regprocedure('public.my_boats()') is not null then
 raise exception 'This upgrade requires the pre-tracking local creation_access schema'; end if;
end $$;
create temporary table upgrade_counts as select
 (select count(*) from auth.users) users_count,
 (select count(*) from public.boats) boats_count,
 (select count(*) from public.series) series_count,
 (select count(*) from public.race_results) results_count;
...
do $$ begin
 if exists(select 1 from upgrade_counts where
 users_count<>(select count(*) from auth.users) or boats_count<>(select count(*) from public.boats)
 or series_count<>(select count(*) from public.series) or results_count<>(select count(*) from public.race_results)) then
 raise exception 'Unexpected change to existing data counts'; end if;
end $$;
insert into supabase_migrations.schema_migrations(version,name,statements) values ('202609160000','local_legacy_tracking_upgrade',array[$upgrade$create function public.can_create_series() returns boolean language sql stable security definer set search_path='' as $$
 select auth.uid() is not null and ((public.creation_access()->>'allowed')::boolean);
$$;
create function public.require_series_creator() returns trigger language plpgsql security definer set search_path='' as $$
begin
...
for each row execute function public.capture_audit_change('id');
$upgrade$]);
insert into supabase_migrations.schema_migrations(version,name,statements) values ('202609160001','boat_invitations',array[$invitation$-- Record provenance separately from profile management. Existing owner_id remains
-- the legacy profile manager; neither field grants permission to sail/track.
alter table public.boats add column created_by uuid references auth.users;
update public.boats set created_by=owner_id;
create function public.record_boat_creator() returns trigger language plpgsql set search_path='' as $$
