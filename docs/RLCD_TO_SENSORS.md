...
| **GPS Module** | UART1 | GPIO44 (RX) | Pin 12 | GPIO17 |
| | | GPIO43 (TX) | Pin 10 | GPIO16 |
| **Discovery Button** | GPIO input | GPIO18 (KEY) | Pin 15 | GPIO0 |
| **Discovery LED** | GPIO output | GPIO2 | Pin 9 | GPIO2 |
| **Anchor Light** | Not assigned | — | — | GPIO25 |
...
VCC         →  Pin 1 (3.3V) or Pin 2 (VBUS 5V)
GND         →  Pin 3 or 4 (GND)
TX          →  Pin 12 — GPIO44      (GPS TX → ESP32 RX)
RX          →  Pin 10 — GPIO43      (ESP32 TX → GPS RX)
```

| Signal | RLCD GPIO | Header P1 | Old DevKitC GPIO | Note |
|---|---|---|---|---|
| RX (GPS TX) | **44** | 12 | 17 | was GPIO6 (not on header) |
| TX (GPS RX) | **43** | 10 | 16 | was GPIO7 (not on header) |
...
| **Anchor Light LED** (masthead control) | Not assigned | — | 25 | GPIO20 is reserved for USB |

- GPIO2 blinks rapidly during BLE discovery, solid when connected.
- Anchor light control is not implemented on this target and has no assigned GPIO.
- GPIO19/GPIO20 are reserved for USB, which is used at runtime for logging and framebuffer capture. Do not wire an anchor-light output to GPIO20.
...
| GPS RX | GPIO17 | GPIO6 (not on header) | **GPIO44** (P1-12) | GPIO6 not on header |
| GPS TX | GPIO16 | GPIO7 (not on header) | **GPIO43** (P1-10) | GPIO7 not on header |
| Discovery LED | GPIO2 | GPIO48 (module-only) | **GPIO2** (P1-9) | GPIO48 not on header |
| Anchor Light | GPIO25 | GPIO47 (module-only) | Not assigned | GPIO20 reserved for USB |
...
- GPIO19/GPIO20 (P1-6/P1-8) are the USB Serial/JTAG pins; both remain reserved for runtime USB.
