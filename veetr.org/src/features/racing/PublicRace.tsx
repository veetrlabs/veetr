...
import {t} from './i18n';
export function PublicRace({series,eventId,embedded=false}:{series:Series;eventId:string;embedded?:boolean}) {
 ...
 return <>
  {!embedded && <><nav className="breadcrumbs" aria-label={t('Breadcrumbs')}><a href={back}>{series.name}</a><span>›</span><span>{event.name}</span></nav>
  <section className="entity-details"><h1>{event.name}</h1><p>{t(event.completed?'Completed':'In progress')} · {t('Starting points')}: {event.startingPoints ?? 0}</p>
